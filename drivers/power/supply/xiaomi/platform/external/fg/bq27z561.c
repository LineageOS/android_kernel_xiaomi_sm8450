...
int fg_read_system_soc(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM, "rsoc %d raw_soc:%d,batt_volt:%d,Cur:%d", soc, raw_soc,
	       bq->batt_volt, bq->batt_curr);
	...
	if (soc <= 1 && g_battmngr_noti) {
		...
		battmngr_notifier_call_chain(BATTMNGR_EVENT_FG,
					     g_battmngr_noti);
	}
	return soc;
}
...
int fg_update_charge_full(struct bq_fg_chip *bq)
{
	...
	bq_dbg(PR_OEM,
	       "raw:%d,last_done:%d,done:%d,syv_done:%d,full:%d,health:%d,cur:%d\n",
	       bq->raw_soc, last_charge_done, bq->charge_done, syv_charge_done,
	       bq->charge_full, bq->health, curr);

	if (bq->fast_mode) {
		term_curr = fg_get_term_current(bq);
		curr = -1 * curr;
		if ((bq->charge_done || syv_charge_done) && !bq->charge_full &&
		    curr <= term_curr) {
			if (bq->raw_soc >= BQ_DEFUALT_FULL_SOC) {
				...
				bq->cell_ov_check = 0;
				vote(bq->fcc_votable,
				     FCCMAIN_FG_CHARGE_FULL_VOTER, true, 0);
			} else {
	...
	if (bq->raw_soc <= BQ_CHARGE_FULL_SOC && !bq->charge_done &&
	    !syv_charge_done && bq->charge_full) {
		if (bq->charge_done)
			goto out;
	...
	if ((bq->raw_soc <= BQ_RECHARGE_SOC) && bq->recharge_done_flag &&
...
static void fg_monitor_workfunc(struct work_struct *work)
{
	...
	if (!bq->fv_votable || !bq->smart_batt_votable) {
		bq_dbg(PR_OEM,
		       "get fv votable or smart_batt_votable failed %d\n");
		return;
	}

	smart_batt_fv =
		get_client_vote(bq->smart_batt_votable, SMART_BATTERY_FV);
	...
	if (!bq->old_hw) {
		...
		if (bq->fast_mode) {
			if (g_battmngr_noti) {
				if (!g_battmngr_noti->mainchg_msg.sw_disable) {
					if (smart_batt_fv == 10 &&
					    ...
					    bq->count <= 2) {
						bq->count++;
					} else if (smart_batt_fv == 15 &&
						   bq->batt_volt >= 4515 &&
						   bq->count <= 2) {
						bq->count++;
					} else if (bq->batt_volt >= 4530 &&
						   smart_batt_fv == 0 &&
						   bq->count <= 1) {
						bq->count++;
					}
			...
			if (bq->batt_volt >= 4480)
				vote(bq->fv_votable, OVER_FV_VOTER, true,
				     NO_FFC_OVER_FV);
		}
	...
	if (g_battmngr_noti) {
		if (!g_battmngr_noti->mainchg_msg.chg_plugin &&
			...
			vote(bq->fcc_votable, FCCMAIN_FG_CHARGE_FULL_VOTER,
			     false, 0);
			vote(bq->fv_votable, OVER_FV_VOTER, false, 0);
			bq->count = 0;
			bq->last_count = 0;

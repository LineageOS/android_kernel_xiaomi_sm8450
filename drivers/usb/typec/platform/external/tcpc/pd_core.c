					   ...
					   struct pd_battery_info *bat_info)
{
	...
	ret = snprintf(mfrs_info->mfrs_string, sizeof(mfrs_info->mfrs_string),
		       "%s", mstring);
	...
	if (ret < 0) {
		...
		bat_cap->bat_design_cap =
			(uint16_t)PD_BCDB_BAT_CAP_RAW(design_cap);
	}
				      ...
				      struct device_node *np)
{
	...
	pd_port->fix_bat_info =
		devm_kzalloc(&pd_port->tcpc->dev,
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		...
		pd_parse_pdata_bat_info(pd_port, sub,
					&pd_port->fix_bat_info[i]);
	}
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		pr_info("%s fix_bat_info[%d].mfrs_info.vid = 0x%x, .mfrs_info.pid = 0x%x, .mfrs_string = %s, .bat_design_cap = %d\n",
			...
			PD_BCDB_BAT_CAP_VAL(pd_port->fix_bat_info[i]
						    .bat_cap.bat_design_cap));
	}
					   ...
					   struct device_node *np)
{
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		pr_info("%s country_info[%d].code = 0x%x, .len = %d\n",
			__func__, i, pd_port->country_info[i].code,
...
static void pd_parse_log_src_cap_ext(struct pd_source_cap_ext *cap)
{
	pr_info("%s vid = 0x%x, pid = 0x%x, xid = 0x%x, fw_ver = 0x%x, hw_ver = 0x%0x\n",
		__func__, cap->vid, cap->pid, cap->xid, cap->fw_ver,
		cap->hw_ver);

	pr_info("%s voltage_regulation = %d, hold_time_ms = %d, compliance = 0x%x, touch_current = 0x%x, peak_current = %d %d %d\n",
		__func__, cap->voltage_regulation, cap->hold_time_ms,
		cap->compliance, cap->touch_current, cap->peak_current[0],
		cap->peak_current[1], cap->peak_current[2]);
				       ...
				       struct device_node *np)
{
	...
	pd_port->id_vdos[2] =
		VDO_PRODUCT(pid, PD_PRODUCT_BCD(pd_port->id_vdos[2]));
...
static void fg_bat_absent_work(struct work_struct *work)
{
	struct pd_port *pd_port =
		container_of(work, struct pd_port, fg_bat_work);
		    ...
		    bool ext, uint16_t count, const uint32_t *data)
{
	...
	if (sop_type == TCPC_TX_SOP)
		msg_hdr_private =
			PD_HEADER_ROLE(pd_port->power_role, pd_port->data_role);
	else
		msg_hdr_private = 0;
	...
	msg_hdr = PD_HEADER_COMMON(msg, pd_rev, msg_id, count, ext,
				   msg_hdr_private);
...
int pd_reply_wait_reject_msg_no_resp(struct pd_port *pd_port)
{
	...
	uint8_t msg = pd_event->msg_sec == PD_DPM_NAK_REJECT ? PD_CTRL_REJECT :
							       PD_CTRL_WAIT;
			 ...
			 uint8_t cnt, uint32_t *data_obj, uint32_t timer_id)
{
	...
	ret = pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF, 1 + cnt,
			       payload);
			  ...
			  uint32_t *data_obj)
{
	...
	return pd_send_sop_data_msg(pd_port, PD_DATA_VENDOR_DEF, 1 + cnt,
				    payload);
}
			...
			uint32_t *payload)
{
	...
	return pd_send_data_msg(pd_port, sop_type, PD_DATA_VENDOR_DEF, cnt,
				payload);
}

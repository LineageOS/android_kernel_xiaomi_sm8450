...
int touch_update_fod_enable_value(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->finger_unlock_status == FOD_STATUS_INVALID ||
	    tcm_hcd->finger_unlock_status == FOD_STATUS_DISABLED ||
 ...
 */
static int touch_get_report_data(unsigned int offset, unsigned int bits,
				 unsigned int *data)
{
	unsigned char mask;
	...
	if (bits == 0 || bits > 32) {
		LOGE(tcm_hcd->pdev->dev.parent, "Invalid number of bits\n");
		return -EINVAL;
	}
...
static void touch_debug_info_print(unsigned int len)
{
	...
	if (len > TOUCH_DEBUG_INFO_BYTES_MAX) {
		LOGE(tcm_hcd->pdev->dev.parent,
		     ...
		     TOUCH_DEBUG_INFO_BYTES_MAX);
	}

	offset = 0;
	for (i = 0; i < len / 2; i++) {
		cnt = snprintf(print_buf + offset,
			       TOUCH_DEBUG_INFO_PRINT_SIZE - offset, " %04x",
			       touch_data->debug_info_data[i]);
		offset += cnt;

		if (offset >= TOUCH_DEBUG_INFO_PRINT_SIZE) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "print buf is too small\n");
			break;
		}
	}
	...
	while (idx < config_size) {
		...
		case TOUCH_GESTURE_DATA:
			...
			while (left_bits) {
				...
				retval = touch_get_report_data(offset, bits,
							       &data);
				if (retval < 0) {
					LOGE(tcm_hcd->pdev->dev.parent,
		...
		case TOUCH_REPORT_DEBUG_INFO:
			...
			memset(touch_data->debug_info_data, 0x00,
			       sizeof(touch_data->debug_info_data));
			temp_data =
				(unsigned char *)&touch_data->debug_info_data[0];
			left_bits = bits;
			i = 0;
			while (left_bits) {
				bits = (left_bits >= 8) ? 8 : left_bits;
			...
			/* print debug info */
			if (touch_data->debug_info_data[0] &
			    TOUCH_DEBUG_INFO_PRINT_MASK) {
 ...
 *    the crc-16 value
 */
static inline unsigned short touch_crc16(unsigned char *p, unsigned int len,
					 unsigned short val)
{
...
static unsigned int touch_report_data_match(void)
{
	...
	if (toch_report_size > 2) {
		...
		crc16 = touch_crc16(touch_report_data, toch_report_size - 2,
				    seed);
		if (crc16 == touch_data->crc16_data) {
			retval = 1;
		}
		LOGD(tcm_hcd->pdev->dev.parent, "crc value (0x%04x, 0x%04x)\n",
		     crc16, touch_data->crc16_data);
	} else {
		LOGE(tcm_hcd->pdev->dev.parent, "Invalid report len(%d)\n",
		     toch_report_size);
	}

exit:
	if (retval == 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Mismatch (0x%04x, 0x%04x)\n",
		     crc16, touch_data->crc16_data);
	}
...
#define SYNA_OBJS_CNT_MAX 15
	...
	if (tcm_hcd->fod_enabled &&
	    (tcm_hcd->finger_unlock_status || tcm_hcd->fod_finger)) {
		if (touch_data->gesture_id ==
			    GESTURE_TOUCH_AND_HOLD_DOWN_EVENT ||
		    touch_data->gesture_id ==
			    GESTURE_TOUCH_AND_HOLD_MOVE_EVENT) {
			tcm_hcd->fod_finger = true;
			touch_fod_down_event();
			goto finger_pos;
		} else if ((touch_data->gesture_id ==
			    GESTURE_TOUCH_AND_HOLD_UP_EVENT) ||
			   ((prev_gesture_id ==
				     GESTURE_TOUCH_AND_HOLD_DOWN_EVENT ||
			     prev_gesture_id ==
				     GESTURE_TOUCH_AND_HOLD_MOVE_EVENT) &&
	...
	} else if (!tcm_hcd->fod_enabled &&
		   tcm_hcd->finger_unlock_status == 0 && tcm_hcd->fod_finger) {
		tcm_hcd->fod_finger = false;
...
finger_pos:
	...
	for (idx = 0; idx < touch_hcd->max_objects; idx++) {
...
#endif
			input_report_key(touch_hcd->input_dev, BTN_TOUCH, 1);
			input_report_key(touch_hcd->input_dev, BTN_TOOL_FINGER,
					 1);
		...
		if ((status == PALM) || (status == FINGER) ||
		    (status == GLOVED_FINGER)) {
...
static int touch_set_input_params(void)
{
	...
	input_set_abs_params(touch_hcd->input_dev, ABS_MT_POSITION_X, 0,
			     touch_hcd->max_x, 0, 0);
	input_set_abs_params(touch_hcd->input_dev, ABS_MT_POSITION_Y, 0,
			     touch_hcd->max_y, 0, 0);
	input_set_abs_params(touch_hcd->input_dev, ABS_MT_TOUCH_MINOR, 0, 100,
			     0, 0);
	input_set_abs_params(touch_hcd->input_dev, ABS_MT_TOUCH_MAJOR, 0, 100,
			     0, 0);
...
static int touch_set_report_config(void)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &touch_hcd->out, length);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval = tcm_hcd->write_message(tcm_hcd, CMD_SET_TOUCH_REPORT_CONFIG,
					...
					&touch_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
		     "Failed to write command %s, retval=%d\n",
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Set touch config done\n");
...
static int touch_set_input_reporting(void)
{
	...
	if (retval < 0) {
		...
		LOGN(tcm_hcd->pdev->dev.parent, "Input parameters unchanged\n");
		goto exit;
	}
...
int touch_early_suspend(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (touch_hcd->suspend_touch)
		LOGE(tcm_hcd->pdev->dev.parent,
		     "enter: touch_early_suspend---true---\n");
...
int touch_suspend(struct syna_tcm_hcd *tcm_hcd)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,
	     "-----enter---- %s, wakeup_gesture[0x%02x]\n", __func__,
	     ((tcm_hcd->wakeup_gesture_enabled) ? 1 : 0));
	...
	if (tcm_hcd->wakeup_gesture_enabled) {
		...
		if (tcm_hcd->nonui_status != 1 &&
		    (tcm_hcd->fod_icon_status || tcm_hcd->aod_enable))
		...
		retval = tcm_hcd->set_dynamic_config(
			tcm_hcd, DC_GESTURE_TYPE_ENABLE, tcm_hcd->gesture_type);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		LOGN(tcm_hcd->pdev->dev.parent,
		     "set DC_IN_WAKEUP_GESTURE_MODE\n");
		retval = tcm_hcd->set_dynamic_config(
			tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE, 1);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
...
int touch_resume(struct syna_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->wakeup_gesture_enabled) {
		...
		LOGN(tcm_hcd->pdev->dev.parent,
		     "clear DC_IN_WAKEUP_GESTURE_MODE\n");
		retval = tcm_hcd->set_dynamic_config(
			tcm_hcd, DC_IN_WAKEUP_GESTURE_MODE, 0);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,

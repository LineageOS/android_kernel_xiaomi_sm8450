...
static struct device_attribute *attrs[] = {
	ATTRIFY(pid),  ATTRIFY(size),	ATTRIFY(type),	  ATTRIFY(rows),
	ATTRIFY(cols), ATTRIFY(hybrid), ATTRIFY(buttons),
};
				    ...
				    const char *buf, size_t count)
{
	...
	if (diag_hcd->pid) {
		diag_hcd->task =
			pid_task(find_vpid(diag_hcd->pid), PIDTYPE_PID);
		if (!diag_hcd->task) {
...
static void diag_report(void)
{
	...
	if (state == PING) {
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, &diag_hcd->ping,
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, &diag_hcd->pong,
					    tcm_hcd->report.buffer.data_length);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,

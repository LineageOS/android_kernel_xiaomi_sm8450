...
#define testing_sysfs_show(t_name)                                             \
	static ssize_t testing_sysfs_##t_name##_show(                          \
...
static struct kobj_attribute *attrs[] = { KOBJ_ATTRIFY(size),
...
static struct device_attribute *attrs[] = { ATTRIFY(size),
					    ATTRIFY(device_id),
					    ATTRIFY(config_id),
...
#if (USE_KOBJ_SYSFS)
										static ssize_t
	testing_sysfs_size_show(struct kobject *kobj,
				struct kobj_attribute *attr, char *buf)
#else
										static ssize_t
	testing_sysfs_size_show(struct device *dev,
...
static int testing_get_limits_bytes(unsigned int *row, unsigned int *col,
				    enum test_code code)
{
	unsigned int rows;
	unsigned int cols;
...
static int testing_copy_valid_data(struct syna_tcm_buffer *dest_buffer,
				   char *src_buf, enum test_code code)
{
	int retval = 0;
	int i, j, n;
	int offset = 0;
	...
	buf_size_bytes =
		testing_get_limits_bytes(&limit_rows, &limit_cols, code);
	if (buf_size_bytes == 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static int testing_parse_csv_data(struct syna_tcm_buffer *dest_buffer,
				  char *src_buf, char *name,
				  enum test_code code)
{
	int retval;
	char *psrc = src_buf;
	...
	if (!dest_buffer || !src_buf || !name) {
		retval = -EINVAL;
		LOGE(tcm_hcd->pdev->dev.parent, "Invalid buf pointer for %s\n",
		     name);
		goto exit;
	}
	...
	if (retval < 0) {
		...
		LOGI(tcm_hcd->pdev->dev.parent, "load PT%d %s data: Success\n",
		     code, name);
	}
...
static int testing_load_testlimits(enum test_code testcode,
				   unsigned int gapdiff)
{
	int retval = 0;
	char *csv_buf = NULL;
	...
	case TEST_PT22_TRANS_RAW_CAP:
		if (gapdiff) {
			sprintf(test_name0, "%s",
				CSV_GAP_DIFF_TESTING_LIMITS_MAX);
			dest_buffer0 = &testing_hcd->pt_hi_limits;
		} else {
	...
	case M9_TOUCH_PANEL_MAKER:
		sprintf(limit_file_name, "%s",
			SYNA_TCM_TESTING_LIMITS_FILE_NAME);
		break;
	case M9_SECOND_TOUCH_PANEL_MAKER:
		sprintf(limit_file_name, "%s",
			SYNA_TCM_SECOND_TESTING_LIMITS_FILE_NAME);
		break;
	default:
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	LOGN(tcm_hcd->pdev->dev.parent, "limit_file_name:%s.\n",
	     limit_file_name);

	retval = request_firmware(&firmware, limit_file_name,
	...
	if (dest_buffer0) {
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, dest_buffer0,
					    buf_size_bytes);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		retval = testing_parse_csv_data(dest_buffer0, csv_buf,
						test_name0, testcode);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to read limts data for %s [PT%d] from csv file\n, retval=%d",
			     test_name0, testcode, retval);
			goto exit;
		}
	}
	...
	if (dest_buffer1) {
		/* allocate the mem for limits buffer */
		retval = syna_tcm_alloc_mem(tcm_hcd, dest_buffer1,
					    buf_size_bytes);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
			     "Failed to allocate memory for PT%d buf, retval=%d\n",
		...
		/* load the limits */
		retval = testing_parse_csv_data(dest_buffer1, csv_buf,
						test_name1, testcode);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
...
static unsigned int testing_save_output(char *out_buf, unsigned int offset,
					char *pstr)
{
	...
	if (out_buf == NULL) {
		LOGI(tcm_hcd->pdev->dev.parent,
		     "Do not support save ouput data\n");
		goto exit;
	}
	...
	if (data_len == (rows * cols * 2)) {
		...
		for (row = 0; row < rows; row++) {
			...
			for (col = 0; col < cols; col++) {
				data = (short)((unsigned short)(buf[idx] &
								0xff) |
					       (unsigned short)(buf[idx + 1]
								<< 8));
				cnt = snprintf(out_buf + sum,
					       SAVE_BUF_SIZE - sum, "%d,",
					       data);
				sum += cnt;
	...
	} else if ((data_len == ((rows + cols) * 4)) &&
		   (testing_hcd->test_item == TEST_PT18_HYBRID_ABS_RAW)) {
		idx = 0;
		cnt = 0;
...
static int testing_run_prod_test_item(enum test_code test_code)
{
	...
	if (tcm_hcd->features.dual_firmware &&
		...
		LOGE(tcm_hcd->pdev->dev.parent, "Identifying mode = 0x%02x\n",
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &testing_hcd->out, 1);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_PRODUCTION_TEST, testing_hcd->out.buf, 1,
		&testing_hcd->resp.buf, &testing_hcd->resp.buf_size,
		&testing_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				   ...
				   unsigned int num_of_reports)
{
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_ENABLE_REPORT, testing_hcd->out.buf, 1,
		&testing_hcd->resp.buf, &testing_hcd->resp.buf_size,
		&testing_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval = tcm_hcd->write_message(
		tcm_hcd, CMD_DISABLE_REPORT, testing_hcd->out.buf, 1,
		&testing_hcd->resp.buf, &testing_hcd->resp.buf_size,
		&testing_hcd->resp.data_length, NULL, 0);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
...
static void testing_copy_resp_to_output(void)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &testing_hcd->output, output_size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval =
		secure_memcpy(testing_hcd->output.buf,
...
static void testing_standard_frame_output(bool image_only)
{
	...
	retval = secure_memcpy(testing_hcd->output.buf,
			       testing_hcd->output.buf_size,
			       &app_info->num_of_buttons[0], header_size,
			       header_size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval =
		secure_memcpy(testing_hcd->output.buf + header_size,
...
static int testing_device_id(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Start testing\n");
	...
	strptr = strnstr(id_info->part_number, device_id_limit,
...
static int testing_pt01_trx_trx_short(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent, "Start testing:%s\n",
	...
	limits_size = testing_get_limits_bytes(&limits_row, &limits_col,
	...
	if (limits_size < testing_hcd->resp.data_length) {
		LOGE(tcm_hcd->pdev->dev.parent, "Mismatching limits data\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_pt03_trx_ground(void)
{
	...
	LOCK_BUFFER(testing_hcd->pt_hi_limits);
	limits_size = testing_get_limits_bytes(&limits_rows, &limits_cols,
...
static int testing_pt05_full_raw(void)
{
	...
	if ((limits_size != frame_size) &&
		...
		LOGE(tcm_hcd->pdev->dev.parent, "Frame size mismatch\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_pt07_dynamic_range(void)
{
	...
	limits_rows = sizeof(pt7_hi_limits) / sizeof(pt7_hi_limits[0]);
	...
	limits_cols = sizeof(pt7_lo_limits[0]) / sizeof(pt7_lo_limits[0][0]);
...
static int testing_pt11_open_detection(void)
{
	...
	if (image_size_words != testing_hcd->resp.data_length / 2) {
		LOGE(tcm_hcd->pdev->dev.parent, "Image size mismatch\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		retval = -EINVAL;
		goto exit;
	}
...
static int testing_pt18_hybrid_abs_raw(void)
{
	...
	if (frame_size != testing_hcd->resp.data_length) {
		LOGE(tcm_hcd->pdev->dev.parent, "Frame size is mismatching\n");
		UNLOCK_BUFFER(testing_hcd->resp);
		retval = -EINVAL;
		goto exit;
	}
	...
	for (col = 0; col < limits_cols; col++) {
		data = (int)(buf[idx] & 0xff) | (int)(buf[idx + 1] << 8) |
		       (int)(buf[idx + 2] << 16) | (int)(buf[idx + 3] << 24);
...
static int testing_reset_open(void)
{
	...
	if (bdata->reset_gpio < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Hardware reset unavailable\n");
		return -EINVAL;
	}
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			data = (unsigned short)(buf[idx] & 0xff) |
			       (unsigned short)(buf[idx + 1] << 8);
			pt22_hi =
				(unsigned short)(pt22_hi_limits[idx] & 0xff) |
				(unsigned short)(pt22_hi_limits[idx + 1] << 8);
			pt22_lo =
				(unsigned short)(pt22_lo_limits[idx] & 0xff) |
...
static int testing_gap_diff(void)
{
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			gap_diff_limits = le2_to_uint(
				&gap_diff_limits_buf[(row * cols + col) * 2]);
			...
			uint_to_le2(&gap_diff_data_buf[(row * cols + col) * 2],
				    diff);
		}
	}
	...
	retval = secure_memcpy(testing_hcd->output.buf,
			       testing_hcd->output.buf_size, gap_diff_data_buf,
...
static int testing_self_test(char *buf)
{
	...
	mask = (1 << TEST_PT1_TRX_TRX_SHORTS) |
	       ...
	       (1 << TEST_PT22_TRANS_RAW_CAP) | (1 << TEST_PT10_DELTA_NOISE) |
	...
	result_flag |=
		(testing_hcd->result) ? (1 << TEST_PT1_TRX_TRX_SHORTS) : 0;
	...
	result_flag |=
		(testing_hcd->result) ? (1 << TEST_PT3_TRX_GROUND_SHORTS) : 0;
	offset = testing_save_output(save_buf, offset, NULL);
	...
	result_flag |=
		(testing_hcd->result) ? (1 << TEST_PT22_TRANS_RAW_CAP) : 0;
	offset = testing_save_output(save_buf, offset, NULL);
	...
	result_flag |=
		(testing_hcd->result) ? (1 << TEST_PT18_HYBRID_ABS_RAW) : 0;
	offset = testing_save_output(save_buf, offset, NULL);
	...
	cnt = snprintf(
		result_info, RESULT_INFO_LEN,
		"Panel Test Result = %s: PT1 = %s  PT3 = %s  PT22 = %s  GAP_DIFF = %s  PT10 = %s  PT18 = %s  PT5 = %s\n",
		(testing_hcd->result) ? "pass" : "fail",
		(result_flag & (1 << TEST_PT1_TRX_TRX_SHORTS)) ? "pass" :
								 "fail",
		(result_flag & (1 << TEST_PT3_TRX_GROUND_SHORTS)) ? "pass" :
								    "fail",
		(result_flag & (1 << TEST_PT22_TRANS_RAW_CAP)) ? "pass" :
								 "fail",
		(gap_diff_test_result) ? "pass" : "fail",
		(result_flag & (1 << TEST_PT10_DELTA_NOISE)) ? "pass" : "fail",
		(result_flag & (1 << TEST_PT18_HYBRID_ABS_RAW)) ? "pass" :
								  "fail",
...
#if TESTING_RESULT_IN_CSV
	if (!iscit) {
		cnt = snprintf(save_buf + offset, SAVE_BUF_SIZE - offset,
			       "%s\n", result_info);
		offset += cnt;
...
exit:
	LOGN(tcm_hcd->pdev->dev.parent, "Panel Test Result = %s\n",
	     (testing_hcd->result) ? "pass" : "fail");
...
static int testing_print_report_data(enum report_type report_type, char *buf)
{
	...
	for (row = 0; row < rows; row++) {
		...
		for (col = 0; col < cols; col++) {
			...
			cnt = snprintf(buf + offset, PAGE_SIZE - offset, pfmt,
				       data);
			offset += cnt;
	...
	if (has_hybrid) {
		pfmt = (report_type == REPORT_RID161) ? (FORMAT_5U) :
							(FORMAT_5D);
		/* print hybrid-x data */
...
static int testing_xiaomi_report_data(int report_type, char *buf)
{
	...
	LOGI(tcm_hcd->pdev->dev.parent, "retval=%d, PAGE_SIZE=%ld\n", retval,
	     PAGE_SIZE);
...
static void testing_report(void)
{
	...
	if (testing_hcd->report_index == 0) {
		retval = syna_tcm_alloc_mem(
			tcm_hcd, &testing_hcd->report,
...
static int testing_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	testing_hcd->sysfs_dir =
		kobject_create_and_add(SYSFS_DIR_NAME, tcm_hcd->sysfs_dir);
	...
	proc_entry = proc_create_data("auto_test", 0660, NULL, &auto_test_ops,
				      tcm_hcd);

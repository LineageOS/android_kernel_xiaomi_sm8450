...
static void device_capture_touch_report(unsigned int count)
{
	...
	case REPORT_TOUCH:
		...
		retval = syna_tcm_alloc_mem(tcm_hcd, &device_hcd->report,
					    remaining_size);
		if (retval < 0) {
...
static int device_capture_touch_report_config(unsigned int count)
{
	...
	if (device_hcd->raw_mode) {
		if (count < 3) {
			LOGE(tcm_hcd->pdev->dev.parent, "Invalid write data\n");
			return -EINVAL;
		}
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &tcm_hcd->config, size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
	...
	retval = secure_memcpy(tcm_hcd->config.buf, tcm_hcd->config.buf_size,
			       data, buf_size, size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			   ...
			   loff_t *f_pos)
{
	...
	if (device_hcd->raw_mode) {
		retval = syna_tcm_alloc_mem(tcm_hcd, &device_hcd->resp, count);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
		...
		retval = tcm_hcd->read_message(tcm_hcd, device_hcd->resp.buf,
					       count);
		if (retval < 0) {
			LOGE(tcm_hcd->pdev->dev.parent,
	...
	if (tcm_hcd->report_touch == NULL) {
		LOGE(tcm_hcd->pdev->dev.parent, "Unable to report touch\n");
		device_hcd->concurrent = false;
	}
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	retval = syna_tcm_alloc_mem(tcm_hcd, &device_hcd->out,
	...
	if (device_hcd->raw_mode) {
		retval = tcm_hcd->write_message(tcm_hcd, device_hcd->out.buf[0],
		...
		retval = tcm_hcd->write_message(
			tcm_hcd, device_hcd->out.buf[0],
			&device_hcd->out.buf[1], count - 1,
			&device_hcd->resp.buf, &device_hcd->resp.buf_size,
			&device_hcd->resp.data_length, NULL, 0);
...
static int device_create_class(void)
{
	...
	if (IS_ERR(device_hcd->class)) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to create class\n");
		return -ENODEV;
	}
...
static int device_init(struct syna_tcm_hcd *tcm_hcd)
{
	...
	device_hcd->device =
		device_create(device_hcd->class, NULL, device_hcd->dev_num,
			      NULL, CHAR_DEVICE_NAME "%d",
			      MINOR(device_hcd->dev_num));
	if (IS_ERR(device_hcd->device)) {
		LOGE(tcm_hcd->pdev->dev.parent, "Failed to create device\n");
		retval = -ENODEV;
